...
| Home Page Reorg Plan B | ✅ | _this commit_ | Home page composition finalized via five new sections: `HomeHeroCarousel` (image rotation from new `home.heroSlides[]` Payload field, legacy `hero_media` deprecated), `HomeRoomsTiles` (new `rooms` collection with three seeded entries: kid/teen/adult, dynamic `/rooms/[slug]` route for detail pages), `HomeBrandStatement` (forest-dark background + `CountUp` animation component, closes FU-Foundation-A motion primitives), `HomeJournalRows` (replaces `HomeJournalTeaser`, new `computeRowOffset` shared helper with 5 unit tests), `HomeShowroomsTeaser` (teaser to showrooms collection). Legacy home sections removed from flow: `HomeFeaturedDesigns`, `HomeShowroomsStrip`, `HomeJournalTeaser`, `HomeInquiryCta` (preserved in `/lab/page.tsx` for reference). `BlurInText` applied to all visible text on home page (word-by-word blur-in reveal on scroll). New `@zhic/ui` shared helper: `computeRowOffset(rowIndex, pageSize)` — computes scroll offset for journal rows math, includes RTL awareness. 25 @zhic/ui tests pass (21 from Phase structure + 5 from journal-rows-math). Full repo typecheck passes. Production build succeeds. All key routes return correct HTTP status codes: `/` → 200, `/rooms/kid|teen|adult` → 200, `/rooms/foo` → 404. Plan: `docs/superpowers/plans/2026-05-19-home-page-b.md`. |
