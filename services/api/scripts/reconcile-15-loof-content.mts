...
async function main() {
  ...
  for (const p of LOOF) if (!(await relExists(p.id, 'tags', 'tags_id', TAG_MODERN))) { if (APPLY) { await insRel(p.id, 'tags', 'tags_id', TAG_MODERN); inserts++ } }
  ...
  console.log('\nD3) Cross-sell — pairsWith + relatedProducts')
  ...
  for (const p of LOOF) { const rel = LOOF.filter((o) => o.id !== p.id).sort((x, y) => x.prio - y.prio).slice(0, RELATED_CAP); for (const r of rel) if (!(await relExists(p.id, 'relatedProducts', 'products_id', r.id))) { if (APPLY) { await insRel(p.id, 'relatedProducts', 'products_id', r.id); inserts++ } } }
